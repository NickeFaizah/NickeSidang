        ...
        <table class="table table-responsive table-striped">
            ...
            <tbody>
                ...
                <tr class="align-middle">
                    <td>{{ ++$y }}</td>
                    <td>
                        <img src="{{ asset('storage/product/' . $x->foto) }}" style="width:100px; height: 100px; border-radius: 50%;">
                    </td>
                    <td>{{ $x->created_at }}</td>
                    <td>{{ $x->sku }}</td>
...
<script>
    ...
    //             $.ajax({
    //                 type: "DELETE"
    //                 , url: "{{ route('destroy.deleteData', ['id' => ':id']) }}".replace(':id', id)
    //                 , dataType: "json"
    //                 , success: function(response) {
    //                     if (response.success) {
    //                         Toast.fire({
    ...
    $(document).ready(function() {
        $('.deleteData').click(function(e) {
            ...
            if (confirm("Apakah Anda yakin ingin menghapus product ini? " + sku + "?")) {
                $.ajax({
                    type: "DELETE"
                    , url: "{{ route('destroy.deleteData', ['id' => ':id']) }}".replace(':id', id)
                    , dataType: "json"
