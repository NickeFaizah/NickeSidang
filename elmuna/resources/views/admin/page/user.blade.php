        ...
        <table class="table table-responsive table-striped">
            ...
            <tbody>
                ...
                <tr class="align-middle">
                    <td>{{ ++$y }}</td>
                    <td>
                        <img src="{{ asset('storage/user/' . $x->foto) }}" style="width:100px; height: 100px; border-radius: 50%; object-fit: cover;">
                    </td>
                    <td>{{ $x->nik }}</td>
                    <td>{{ $x->created_at }}</td>
                    <td>{{ $x->name }}</td>
...
<script>
    ...
    //             $.ajax({
    //                 type: "DELETE"
    //                 , url: "{{ route('destroy.DataUser', ['id' => ':id']) }}".replace(':id', id)
    //                 , dataType: "json"
    //                 , success: function(response) {
    //                     if (response.success) {
    //                         Toast.fire({
    ...
    $('.deleteData').click(function(e) {
        ...
        if (confirm("Apakah Anda yakin untuk menghapus karyawan ini?")) {
            $.ajax({
                type: "DELETE"
                , url: "{{ route('destroy.DataUser', ['id' => ':id']) }}".replace(':id', id)
                , dataType: "json"
